...
glmrao <- glmmTMB(SES ~ Trait*TIME_OF_YEAR + (1|SITE) + (1|Trait), data=comm_roa2)
anova(glmrao0,glmrao)
Anova(glmrao)
summary(glmrao)

emmeans(glmrao, ~Trait, infer=T)
emmeans(glmrao, ~TIME_OF_YEAR, infer=T)

#FEEDING NICHE AND TRAIT MATCHING########################################################################
#LOAD IN PLANT DATA AND PROCESS PLANT DATA
sla<-read.csv('Leaf SLA_2022.csv') %>% select(-c(DRY_WT,AREA))
ldmc<-read.csv('Plant CNP.csv') %>% select(-c(WET_WT,DRY_WT))
cndata<-read.csv('Hahn Plant TC TN results march 2023.csv')
...
exp<-read.csv('GHopp Feeding Trials_2022.csv')
