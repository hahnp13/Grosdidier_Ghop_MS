...
comm0check <-comm %>% 
  ...
  select(-c(DATE,HABITAT,GENUS,SEX,AGE,IDed,MORPH.CODE,SPP.NOTES,X)) %>% 
...
comm1<-comm %>% 
  mutate(DATE=as.Date(DATE, format = '%m/%d/%Y')) %>% 
  filter(DATE < as.Date('2022-01-01'), IDed=='Y', TIME.OF.YEAR=='EARLY') %>% 
  mutate(SUBFAMILY=ifelse(GENUS=='MELANOPLUS','MEL',SUBFAMILY),
         SPECIES=paste(substr(GENUS,1,2),substr(SPECIES,1,2),sep = "")) %>% 
  select(-c(DATE,GENUS,SEX,AGE,IDed,MORPH.CODE,SPP.NOTES,X)) %>% 
...
comm1$richness<-hill_taxa(comm1[3:17], q=0)
comm1$shannon<-hill_taxa(comm1[3:17], q=1)
comm1$simpsonsinv<-hill_taxa(comm1[3:17], q=2)
comm1$abund<-rowSums(comm1[3:17])

ggplot(comm1, aes(x=HABITAT, y=abund)) +
  geom_boxplot()

ggplot(comm1, aes(x=MERO, y=APSP))+
  geom_point()+
  facet_wrap(~HABITAT)
ggplot(comm1, aes(x=AMMY, y=APSP))+
  geom_point()+
  facet_wrap(~HABITAT)

comm2 <- comm1 %>% group_by(HABITAT,SITE) %>% 
  pivot_longer(cols=3:16, names_to = "Spp", values_to = "count")

ggplot(comm2 %>% filter(Spp=='ACCA'|Spp=='AMMY'|Spp=='APSP'|Spp=='MERO'|Spp=='ORPE'), 
       aes(x=Spp, y=count, color=HABITAT)) +
  geom_boxplot()
...
comm2<-comm1 %>%  
  .[3:16] %>% select(order(colnames(.))) %>% as.matrix()
...
for (i in 1:999) {#For each row in the matrix (for each site)
  #No longer do this: select randomly, as many species as the species richness of the site:
  # Randomize only the trait x species matrix
  comm3rand <- comm3
...
glmrao0 <- glmmTMB(SES ~ Trait*TIME_OF_YEAR*TIME_SINCE_FIRE_DAYS + (1|SITE/Trait), data=comm_roa2, dispformula = ~Trait)
...
## Microniche Breadth ####
nmat <- exp3 %>% select(GRASSHOPPER_SPECIES, ARBE:VAMY) %>% 
  rowwise() %>% 
...
lm.IS_LDMC <- glmmTMB(LDMC_M ~ IS + (1|SUBFAMILY), data=link %>% filter(LDMC_M<420) )
...
lm.BV_LDMC <- glmmTMB(SLA_M ~ IS + (1|SUBFAMILY), data=link )
summary(lm.BV_LDMC)
coef(lm.BV_LDMC)
Anova(lm.BV_LDMC)
#anova(lm.IS_LDMC, ddf="Kenward-Roger")
...
lm.CN_CN <- glmmTMB(CN_RATIO_PLANT ~ CN_RATIO + (1|SUBFAMILY), data=link )
summary(lm.CN_CN)
coef(lm.CN_CN)
Anova(lm.CN_CN)
#anova(lm.IS_LDMC, ddf="Kenward-Roger")
...
### CN and LDMC ####
CN_tab1 <- ggpredict(lm.CN_CN, terms = c("CN_RATIO [4.09:4.6, by=.01]"), 
                     ci.lvl = .95, type = "fe") %>% as.data.frame() 
...
ghoppCN_CN <- link %>% ggplot() +
  ...
  geom_point(data=link, aes(x=CN_RATIO, y=CN_RATIO_PLANT, color=SUBFAMILY), size=3) +
  scale_color_viridis(discrete=T, option = 'plasma') + 
  #guides(color = guide_legend(override.aes=list(shape = 16))) +
  theme_bw(base_size = 16) +
  xlab("Grasshopper C:N Ratio") +
  ylab("Weighted mean Plant C:N Ratio in diet") +
  annotate(geom="text", label="B)", x=4.1, y=41, size=6)+
  annotate(geom="text", label="R2m=0.16", x=4.2, y=30, size=5)+
  annotate(geom="text", label="R2c=0.58", x=4.2, y=28.5, size=5)+
  annotate(geom="text", label="p < 0.01", x=4.2, y=27, size=5)+
  theme(
    axis.text = element_text(color = 'black'),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
ghoppBV_CN <- link %>% ggplot() +
  ...
  geom_point(data=link, aes(x=BV, y=CN_RATIO_PLANT, color=SUBFAMILY), size=3) +
  scale_color_viridis(discrete=T, option = 'plasma') + 
  #guides(color = guide_legend(override.aes=list(shape = 16))) +
  theme_bw(base_size = 16) +
  xlab("Grasshopper body volume (mm3)") +
  ylab("Weighted mean Plant C:N Ratio in diet") +
  annotate(geom="text", label="C)", x=4, y=41, size=6)+
  annotate(geom="text", label="R2m=0.06", x=5.2, y=41, size=5)+
  annotate(geom="text", label="R2c=0.89", x=5.2, y=39.5, size=5)+
  annotate(geom="text", label="p = 0.06", x=5.2, y=38, size=5)+
  theme(
    axis.text = element_text(color = 'black'),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
ggsave("linkplot.tiff", linkplot, width=14, height=6, units="in", dpi=600, compression = "lzw")
